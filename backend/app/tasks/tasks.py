...
def expire_otp(delivery_id: int):
    ...
    logger.info(f"Ã¢ÂÂ° OTP expired for delivery {delivery_id}")
...
def send_push_notification(user_id: int, title: str, body: str):
    ...
    logger.info(f"Ã°Å¸â€œÂ² Push notification to user {user_id}: {title}")
...
def send_sms(phone: str, message: str):
    ...
    logger.info(f"Ã°Å¸â€œÂ± SMS to {phone}: {message}")
...
def aggregate_telemetry(robot_id: int):
    ...
    logger.info(f"Ã°Å¸â€œÅ  Aggregating telemetry for robot {robot_id}")
...
def robot_health_check():
    ...
    logger.info("Ã°Å¸ÂÂ¥ Running fleet health check")
